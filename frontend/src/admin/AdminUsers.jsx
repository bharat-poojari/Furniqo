...
const AdminUsers = () => {
  ...
  const handleToggleStatus = async (user) => {
    try {
      // Update user status via profile update
      await apiWrapper.updateUserProfile(user._id, { isActive: !user.isActive });
  ...
  const handleExportUsers = () => {
    const csv = [
      ...
      ...users.map(u => [u.name, u.email, u.role, u.isActive !== false ? 'Active' : 'Inactive', new Date(u.createdAt).toLocaleDateString()])
              ...
              {paginatedUsers.length === 0 ? (
                ...
                paginatedUsers.map((user, idx) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span className={`inline-block px-2 py-1 text-xs rounded-full ${user.isActive !== false ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {user.isActive !== false ? <FiUserX className="h-4 w-4" /> : <FiUserCheck className="h-4 w-4" />}
                        </button>
                        <button
                          onClick={() => {
                            setSelectedUser(user);
